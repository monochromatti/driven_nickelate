...
sns.lineplot(mapping, x="pp_delay", y="delta_cond", color=colors("deep_gray"), marker="o", ax=ax)
...
ax["signal"].plot(*best_lines.select("time", "multiexpo"), color=colors("deep_gray"), zorder=-1)
sns.scatterplot(
    mapping, x="pp_delay", y="delta_cond", ax=ax["signal"], s=25, lw=1, color=colors("deep_gray")
)
ax["signal"].set(
    yscale="linear",
    ylim=(0, 1e5),
...
ax["pnpa"].fill_between(time, signal, clip_on=False, fc=colors("deep_gray"), ec="none", alpha=0.1)
...
```{python}
# | label: "fig-decay-offset"
# | fig-cap: "**a**. Decay dynamics with biexponential fit in colors("deep_gray"). The differential voltage $\\Delta V_\\text{pr}$ on the photodiodes is proportional to the pump-induced change in the transmitted electric field of the probe, as measured at a single time slice on the probe waveform. **b** The photodiode voltage $V_\\text{pu}$ is proportional to the electric field of the incident pump pulse. **c** Temperature dependence of $\\Delta V_\\text{pr}$ at a fixed pump-probe delay of 13 ps."
...
sns.lineplot(data_1D, x="time", y="model", color=colors("deep_gray"), ls="--", ax=axes["probe"])
