...
def process_fourier(waveforms: Dataset) -> Dataset:
    ...
    spec = (
        spec.select_data(
            col("X.real", "X.imag").complex.struct().alias("X[c]"),
            col("X_ref.real", "X_ref.imag").complex.struct().alias("X_ref[c]"),
        )
        .select_data(
        ...
        .with_columns(
            ...
            ccol("t[c]").phase().alias("t.pha"),
        )
        .filter(col("freq").is_between(0.3, 2.5))
        .with_columns(
            ccol("t.pha").unwrap_phase().over("temperature", "direction"),
        )
    )
    return spec
...
def calculate_conductivity(
    ...
    spectra = (
        ...
        .with_columns(pl.col("prop_factor").complex.struct())
        .with_columns((col("t[c]").complex.multiply("prop_factor[c]")).alias("t[c]"))
    )
    ...
    spectra = spectra.with_columns(sigma.alias("sigma[c]"))
    return spectra
...
if __name__ == "__main__":
    ...
    DATAFILES["spectra"].write(spec.filter(col("freq") < 4).complex.unnest())
