...
def process_fourier(waveforms) -> Dataset:
    ...
    spectra = Dataset(spectra, "freq", waveforms.id_vars).select_data(
        col("X.real", "X.imag").complex.nest(),
        col("X.sem.real", "X.sem.imag").complex.nest(),
    )
    spec_ref = (
    ...
    spectra = spectra.join(
        ...
        col("X[c]")
        .complex.divide(col("X[c]_ref"))
        .over("temperature", "direction")
        .alias("t[c]"),
        col("X.sem[c]")
        .complex.divide(col("X.sem[c]_ref"))
        .over("temperature", "direction")
        .alias("t.sem[c]"),
    )
...
def calculate_conductivity(
    ...
    spectra = (
        spectra.with_columns(
            pl.struct(k0.alias("real"), pl.lit(0).alias("imag")).alias("k0[c]"),
        )
        .with_columns(
            ccol("k0[c]").alias("prop_factor[c]"),
        )
        .select_data(
            (ccol("t[c]").exp()).alias("t[c]"),
            col("n_sub[c]"),
        )
    )
...
if __name__ == "__main__":
    ...
    DATAFILES["spectra"].write(spectra.complex.unnest())
    ...
    DATAFILES["film_conductivity"].write(cond.complex.unnest())
