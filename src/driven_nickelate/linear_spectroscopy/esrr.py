...
def process_waveforms() -> Dataset:
    ...
    lsat_data = (
        ...
        .filter(
            ...
            & (pl.col("date").is_between(pl.date(2023, 1, 23), pl.date(2023, 1, 24)))
        )
...
if __name__ == "__main__":
    ...
    spectra = spectra.select_data(
        ...
        col("X_sub.real", "X_sub.imag").complex.struct().alias("X_sub[c]"),
    )
    spectra = spectra.with_columns(
        (ccol("X[c]") / ccol("X_sub[c]")).over("temperature").alias("t[c]")
    )
    DATAFILES["spectra"].write(spectra.filter(col("freq") < 4).complex.unnest())
