...
def relativize_spectra_meas(spectra_meas) -> Dataset:
    spectra_ref = spectra_meas.filter(
        ...
        col("X.real", "X.imag").complex.struct("X_ref[c]"),
    )
    ...
    trel_meas = (
        trel_meas.with_columns(
            ((ccol("X[c]") - ccol("X_ref[c]")) / ccol("X_ref[c]"))
            .conj()  # <--- Unknown reason for inverted sign of imaginary part
            .alias("t.reldiff[c]")
        )
