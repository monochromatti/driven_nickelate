...
def process_fourier(waveforms: Dataset) -> Dataset:
    return (
        ...
        .select_data(
            col("x").complex.struct(),
            col("x_ref").complex.struct(),
        )
        .select_data(
            col("x[c]").complex.divide(col("x_ref[c]")).alias("t[c]"),
...
if __name__ == "__main__":
    ...
    spectra_calc = (
        ...
        .select_data(col("t").complex.struct())
        .sort()
    )
    ...
    spectra0 = spectra_calc.filter(col("cond_gap") == col("cond_film")).select(
        col("cond_film"), col("freq"), col("t[c]").alias("t0[c]")
    )
    spectra_calc = (
        ...
        .select_data(
            ...
            .over("cond_film")
            .alias("t.reldiff[c]"),
            (ccol("t[c]") / ccol("t0[c]")).over("cond_film").alias("t[c]"),
        )
        .sort()
    )

    # sns.relplot(
    #     spectra_calc.complex.unnest().filter(
    ...
    # DATAFILES["trel_joint"].write(trel_joint.complex.unnest())
