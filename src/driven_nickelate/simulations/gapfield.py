...
def import_data(data_path):
    data = (
        ...
        .sort("cond.real", "freq")
        .with_columns(pl.col("Ey").complex.struct(), pl.col("E0y").complex.struct())
        .select(
            pl.col("freq"),
            pl.col("cond.real").cast(pl.Float64).alias("cond"),
            pl.col("Ey[c]").complex.phase().alias("Ey.pha"),
            pl.col("Ey[c]").complex.modulus().alias("Ey.mag"),
            pl.col("E0y[c]").complex.phase().alias("E0y.pha"),
...
if __name__ == "__main__":
    ...
    pnpa_data = pnpa_data.with_columns(
        pl.col("value").complex.struct().complex.modulus().alias("mag")
    )
    pnpa_data = (
        pl.read_csv(
