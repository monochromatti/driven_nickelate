...
def relativize_meas(spectra_meas) -> Dataset:
    ...
    trel_meas = (
        ...
        .select_data(
            ((ccol("X[c]") - ccol("X[c]_ref")) / ccol("X[c]_ref")).alias("t.reldiff[c]")
        )
...
def relativize_calc(spectra_calc) -> Dataset:
    ...
    spectra_calc = spectra_calc.select_data(
        col("t.real", "t.imag").complex.struct("t[c]")
    )
...
def compute_error(trel_joint: Dataset) -> Dataset:
    ...
    signal_expr = ccol("t_meas.reldiff[c]").squared_modulus()
    error = (
...
if __name__ == "__main__":
    ...
    solution = minimize(trel_joint, error).with_columns(
        (col("t_meas.reldiff[c]") - col("t_calc.reldiff[c]")).alias("residual[c]")
    )
