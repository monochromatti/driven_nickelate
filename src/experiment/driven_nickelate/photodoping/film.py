...
if __name__ == "__main__":
    ...
    spectra = Dataset(spectra.complex.struct(), "freq", waveforms.id_vars)
    spectra = spectra.with_columns(
        col("dX[c]").complex.divide(col("X[c]")).alias("t.reldiff[c]"),
        (
            2 / (377 * 12e-9) * col("dX[c]").complex.divide(col("dX[c]") + col("X[c]"))
        ).alias("dsig[c]"),
    )
