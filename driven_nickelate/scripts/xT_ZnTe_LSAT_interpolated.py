...
ref_files = pd.read_csv(os.path.join(parent_dir, 'File lists/xT_ZnTe_LSAT.dat'), comment='#')
...
for i in range(1, df_ref.shape[0]):
    ref_data_interp[i, 1:] = np.interp(tempr_list, ref_files['Get temperature'], df_ref.drop('Delay', axis=1).loc[i])

pd.DataFrame(ref_data_interp, columns=['Delay', *list(tempr_list)]).to_csv(os.path.join(parent_dir, 'Processed data/xT_ZnTe_LSAT_interp.dat'), index=False)