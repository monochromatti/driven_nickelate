...
for folder in ['Raw data/21.01.23', 'Raw data/22.01.23', 'Raw data/23.01.23']:
    paths += [os.path.join(folder, fn) for fn in os.listdir(folder) if re.split('_', fn)[1] == file_id]
...
pd.DataFrame(data={'Pump-probe delay (ps)': ppdel, 'Path': paths}).to_csv('File lists/xDelay_ChoppedProbe.dat', index=False, sep=';')