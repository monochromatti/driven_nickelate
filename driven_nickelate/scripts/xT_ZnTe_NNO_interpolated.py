...
ref_files = pd.read_csv(os.path.join(parent_dir, 'File lists/xT_ZnTe_NNO.dat'), comment='#')

read_kwargs = dict(names=['Delay', 'X', 'SEM X', 'Y', 'Y SEM'], usecols=list(range(5)), sep='\t', header=None, comment='#')
...
for i in range(data_warming.shape[0]):
    interp_warming[i, :] = interp1d(ref_files['Get temperature'].loc[idx_warming], data_warming[i, :], **interp_kwargs)(tempr_list)

interp_cooling = np.zeros((data_cooling.shape[0], len(tempr_list)))
for i in range(data_cooling.shape[0]):
    interp_cooling[i, :] = interp1d(ref_files['Get temperature'].loc[idx_cooling][::-1], data_cooling[i, :], **interp_kwargs)(tempr_list)

pd.DataFrame(np.c_[df['Delay'], interp_warming], columns=['Delay', *list(tempr_list)]).to_csv(os.path.join(parent_dir, 'Processed data/xT_ZnTe_NNO_interp_warming.dat'), index=False)
pd.DataFrame(np.c_[df['Delay'], interp_cooling], columns=['Delay', *list(tempr_list)]).to_csv(os.path.join(parent_dir, 'Processed data/xT_ZnTe_NNO_interp_cooling.dat'), index=False)