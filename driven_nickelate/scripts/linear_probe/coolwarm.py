...
from support_functions import (nearest_index, c_const, read_kwargs,
                               time_window, get_slice)
...
split = lambda ndarr: np.transpose([np.ravel(arr) for arr in np.split(ndarr, 4, axis=2)])
df_time = pd.DataFrame(split(waveforms), columns=['Direction','Temperature','t','EOS'])
df_freq = pd.DataFrame(split(spectra), columns=['Direction','Temperature','f','FFT EOS'])
