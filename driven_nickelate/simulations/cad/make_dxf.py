...
parser = argparse.ArgumentParser(description='Creates .dxf of electric split-ring resonator array')
parser.add_argument('params_file', type=str, help='JSON file with structural parameters')
parser.add_argument('-png', '--store', action='store_true', help='Save a .png output of the design')
...
if __name__ == '__main__':
    ...
    # Internal perimeter
    Ri = R * (1 - 2*np.sqrt(2)*lw/np.sqrt((Lx/2)**2 + (Ly/2)**2)) # Reduced radius
    xi = Lx/2-lw
    yi = Ly/2-lw
    inner_polyline =  [
                        [-xi,       yi-Ri,       b],
    ...
    perimeter_coords = [(-(Lx + 2*dx)/2, -(Ly + 2*dy)/2),
                        (+(Lx + 2*dx)/2, -(Ly + 2*dy)/2),
                        (+(Lx + 2*dx)/2, +(Ly + 2*dy)/2),
                        (-(Lx + 2*dx)/2, +(Ly + 2*dy)/2),
                        (-(Lx + 2*dx)/2, -(Ly + 2*dy)/2)]
