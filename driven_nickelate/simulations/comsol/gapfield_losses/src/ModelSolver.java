    ...
    public static void runParameterSweep(Model model, ModelNode component, SolverSequence sol, Properties prop) {
        ...
        for (int i = 0; i < num_params; i++) {
            ...
            if (recordSpectral) {
                ...
                model.result().numerical("gap_integral").appendResult();
                model.result().export("params_export").set("filename", spectralData);
                model.result().export("params_export").run();
            }
            if (recordSpatial) {
                int dotIndex = spatialData.lastIndexOf(".");
                String filename = spatialData.substring(0, dotIndex) + "_" + i + spatialData.substring(dotIndex);
                model.result().export("spatial_data").set("filename", filename);
