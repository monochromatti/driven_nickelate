...
def relativize_spectra_meas(spectra_meas) -> Dataset:
    spectra_ref = (
        ...
        .select(col("freq"), col("X_ref.real", "X_ref.imag").complex.struct())
    )
    trel_meas = (
        spectra_meas.select_data(col("X.real", "X.imag").complex.struct())
        .join(spectra_ref, on="freq")
        .with_columns(
            ((ccol("X[c]") - ccol("X_ref[c]")) / ccol("X_ref[c]")).alias("t.reldiff[c]")
        )
...
def relativize_spectra_calc(spectra_calc) -> Dataset:
    spectra_calc = spectra_calc.select_data(col("t.real", "t.imag").complex.struct())
    ...
    trel_calc = (
        ...
        .with_columns(
            ((ccol("t[c]") - ccol("t[c]_ref")) / ccol("t[c]_ref")).alias("t.reldiff[c]")
        )
...
def compute_error(trel_joint: Dataset) -> Dataset:
    ...
    error_expr = (
        ccol("t_meas.reldiff[c]") - ccol("t_calc.reldiff[c]")
    ).squared_modulus()
    error = (
...
if __name__ == "__main__":
    ...
    DATAFILES["solution"].write(solution.complex.unnest())
