...
def process_fourier(waveforms: Dataset) -> Dataset:
    ...
    spectra_meas = spectra_meas.select_data(
        col("dX.avg.real", "dX.avg.imag").complex.struct(),
        col("Xeq.avg.real", "Xeq.avg.imag").complex.struct(),
    ).select_data((ccol("dX.avg[c]") / ccol("Xeq.avg[c]")).alias("t.reldiff[c]"))
    return spectra_meas
...
def join_datasets(df_meas, df_calc):
    return df_meas.select(
        col("freq"),
        col("pp_delay"),
        col("t.reldiff[c]").alias("t_meas.reldiff[c]"),
    ).join(
        df_calc.select(
            col("freq"),
            col("cond_gap"),
            col("t.reldiff[c]").alias("t_calc.reldiff[c]"),
        ),
        on="freq",
    )
...
if __name__ == "__main__":
    ...
    DATAFILES["trel_joint"].write(
        trel_joint.sort_columns().complex.unnest(),
    )
    ...
    solution = trel_joint.join(mapping, on=["pp_delay", "cond_gap"]).complex.unnest()
