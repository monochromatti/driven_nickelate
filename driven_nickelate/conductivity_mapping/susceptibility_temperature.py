...
def process_fourier(waveforms: Dataset) -> Dataset:
    spectra_meas = pl_fft(waveforms, "time", waveforms.id_vars).complex.struct()
    spectra_meas = Dataset(spectra_meas, "freq", waveforms.id_vars)
    spectra_meas = spectra_meas.sort(auto=True).select_data(
        ccol("dX[c]").divide(col("X_eq[c]")).alias("t.reldiff[c]")
    )
    return spectra_meas
...
def relativize_calc(spectra_calc) -> Dataset:
    ...
    spectra_reference = (
        ...
        .drop("cond_gap")
        .select_data(col("t.real", "t.imag").complex.struct("t_ref"))
    )
    ...
    trel_calc = (
        ...
        .with_columns(
            ((ccol("t[c]") - ccol("t_ref[c]")) / ccol("t_ref[c]")).alias("t.reldiff[c]")
        )
...
def solve_bilevel(spectra_calc, spectra_meas):
    ...
    DATAFILES["solution_free"].write(solution.complex.unnest())
    ...
    DATAFILES["solution_rigid"].write(solution.complex.unnest())
...
def solve_fixed(spectra_calc, spectra_meas):
    ...
    DATAFILES["solution_fixed"].write(solution.complex.unnest())
